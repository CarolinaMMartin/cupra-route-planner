...
CREATE INDEX idx_import_batches_tipo_estado
  ON public.import_batches (tipo, estado, created_at DESC);
CREATE INDEX idx_import_batches_sha256
...
CREATE INDEX idx_import_staging_rows_batch
  ON public.import_staging_rows (batch_id, tipo_fila, numero_fila);
CREATE INDEX idx_import_staging_rows_expires_at
...
-- Hace atomico el reemplazo de ventas: si cualquier fila falla, PostgreSQL
-- revierte tanto el DELETE como el INSERT y el historico anterior queda intacto.
...
BEGIN
  ...
  ELSE
    -- En carga incremental reemplaza solamente las claves ya existentes.
