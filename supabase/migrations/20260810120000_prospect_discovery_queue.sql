...
-- Solo se conserva place_id (exceptuado de las restricciones de cache), junto con
-- metadatos propios del flujo comercial.
...
-- La policy histórica llamada "Service role" no tenía TO service_role y, por
-- lo tanto, habilitaba escrituras a cualquier rol. Se reemplaza por permisos
-- explícitos: lectura autenticada, alta manual autenticada y service_role real.
...
-- Un registro publico nunca puede elegir privilegios. Toda cuenta nueva nace
-- como vendedor inactivo y un asignador la habilita/promueve desde la plataforma.
...
-- La tabla profiles habia quedado con RLS deshabilitado en una migracion
-- historica. Se restablece el limite: cada usuario solo ve su perfil y un
-- asignador activo puede administrar el conjunto completo.
...
-- Las policies restrictivas se combinan con todas las policies permisivas
-- historicas y evitan que una cuenta deshabilitada use estas tablas por REST.
